  ...
  return <Card><CardHeader><CardTitle className="flex items-center gap-2"><MessageSquare className="size-5" />{t("companions.chatHistory")}</CardTitle><CardDescription>{t("companions.chatHistoryDesc")}</CardDescription></CardHeader><CardContent>
    {conversations.isLoading ? <Skeleton className="h-64 w-full" /> : !selected ? <p className="py-10 text-center text-sm text-muted-foreground">{t("companions.noConversations")}</p> : <div className="grid min-h-96 gap-4 lg:grid-cols-[280px_1fr]">
      ...
      <div className="flex min-h-96 flex-col rounded-md border bg-muted/30">
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-4">{messages.isLoading ? <Skeleton className="h-40 w-full" /> : ordered.length === 0 ? <p className="py-10 text-center text-sm text-muted-foreground">{t("companions.noMessages")}</p> : ordered.map((message) => { const fromUser = message.sender_type === "user"; return <div key={message.id} className={`flex ${fromUser ? "justify-end" : "justify-start"}`}><div className={`max-w-[80%] rounded-md px-3 py-2 text-sm ${fromUser ? "bg-primary text-primary-foreground" : "border bg-background"}`}><div className="mb-1 text-[11px] opacity-70">{fromUser ? t("companions.user") : companionName} · {message.message_type}</div>{message.content && <div className="whitespace-pre-wrap break-words">{message.content}</div>}{message.media_url && <a href={message.media_url} target="_blank" rel="noreferrer" className="mt-1 block underline">{t("companions.media")}</a>}<div className="mt-1 text-[10px] opacity-60">{formatDate(message.created_at)} · {message.delivery_status}</div></div></div>; })}</div>
