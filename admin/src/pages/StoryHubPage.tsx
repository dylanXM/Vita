...
export function StoryHubPage() {
  ...
  return <div className="space-y-6"><PageHeader title={t("storyHub.title")} description={t("storyHub.desc")} actions={<Select value={active ?? ""} onValueChange={(v) => { setEnvironment(v as Environment); setEditing(null); }}><SelectTrigger className="w-40"><SelectValue /></SelectTrigger><SelectContent>{ENVIRONMENTS.map((x) => <SelectItem key={x} value={x}>{t(`billing.env.${x}`)}</SelectItem>)}</SelectContent></Select>} />
    <Card><CardHeader><CardTitle>{t("storyHub.rules")}</CardTitle><CardDescription>{t("storyHub.customLimitHint")}</CardDescription></CardHeader><CardContent>{rules && <div className="grid gap-4 md:grid-cols-3">{(["free_chapter_limit","custom_background_limit","storyboard_unlock_chapters","storyboard_panel_count","chapter_coins","storyboard_coins"] as const).map((key) => <Field key={key} label={t(`storyHub.${({free_chapter_limit:"freeLimit",custom_background_limit:"customLimit",storyboard_unlock_chapters:"unlock",storyboard_panel_count:"panels",chapter_coins:"chapterCoins",storyboard_coins:"storyboardCoins"} as const)[key]}`)}><Input type="number" min={key === "free_chapter_limit" || key === "custom_background_limit" ? 0 : 1} value={rules[key]} onChange={(e) => setRules({...rules,[key]:Number(e.target.value)})}/></Field>)}<div className="flex items-end"><Button disabled={saveRules.isPending} onClick={() => saveRules.mutate()}><Save />{t("storyHub.saveRules")}</Button></div></div>}</CardContent></Card>
