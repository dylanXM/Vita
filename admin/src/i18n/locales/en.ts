// English — the master key set. Every other locale mirrors this structure.
const en = {
  brand: "Vita Admin",

  common: {
    loading: "Loading…",
    language: "Language",
    toggleTheme: "Toggle theme",
    admin: "Administrator",
    signOut: "Sign out",
    somethingWrong: "Something went wrong",
    notFound: "This page doesn't exist.",
    backToDashboard: "Back to dashboard",
    refresh: "Refresh",
    failedToLoad: "Failed to load.",
  },

  nav: {
    overview: "Overview",
    dashboard: "Dashboard",
    management: "Management",
    users: "Users",
    companions: "Characters",
    agent: "AI companion",
    lifeEngine: "Life Engine",
    modelServices: "Model services",
    mediaModels: "Model routing",
    storage: "Media storage",
    invitationSettings: "Invitation settings",
    billing: "Billing",
    subscriptionPlans: "Subscription plans",
    coinPacks: "Coin packs",
    billingActivity: "Purchases & ledger",
    creditProducts: "Credit experiences",
    appContent: "App content", onboarding: "Onboarding", whatsNew: "What's new", socialLinks: "Social media links", legalDocuments: "Legal documents", aiPets: "AI Pets", storyHub: "Story Hub",
  },

  login: {
    subtitle: "Sign in with an administrator account",
    email: "Email",
    password: "Password",
    signIn: "Sign in",
    signingIn: "Signing in…",
    emailInvalid: "Enter a valid email",
    passwordRequired: "Password is required",
    welcome: "Welcome back",
    notAdmin: "This account is not an administrator.",
    failed: "Sign-in failed",
    hint: "Seeded from VITA_ADMIN_EMAIL / VITA_ADMIN_PASSWORD.",
  },

  dashboard: {
    title: "Dashboard",
    desc: "Live counts read straight from the Vita database.",
    users: "Users",
    usersSub: "{{admins}} administrators",
    newUsers7d: "New users (7d)",
    newUsers7dSub: "Registered in the last 7 days",
    companions: "Companions",
    companionsSub: "{{n}} new in the last 7 days",
    conversations: "Conversations",
    conversationsSub: "{{n}} new in the last 7 days",
    messages: "Messages",
    messagesSub: "{{n}} new in the last 7 days",
    memories: "Memories",
    memoriesSub: "Long-term memory entries",
    lifeEvents: "Life events",
    lifeEventsSub: "{{n}} scheduled today",
    systemInfo: "System",
    apiVersion: "API version",
    environment: "Environment",
    generatedAt: "Collected at",
    dataNote: "Every figure is a live COUNT over the current tables. Refresh to re-read.",
  },

  users: {
    title: "Users",
    desc: "Manage regular user accounts.",
    searchPlaceholder: "Search by email…",
    allRoles: "All roles",
    allStatus: "All statuses",
    environment: "Environment",
    allEnvironments: "All environments",
    envDev: "dev (development)",
    envBeta: "beta (pre-release)",
    envProd: "prod (production)",
    status: "Status",
    roleUser: "User",
    roleAdmin: "Admin",
    statusActive: "Active",
    statusBanned: "Banned",
    email: "Email",
    password: "Password",
    passwordHint: "Leave blank to keep unchanged",
    role: "Role",
    timezone: "Timezone",
    createdAt: "Created",
    updatedAt: "Updated",
    actions: "Actions",
    view: "View",
    edit: "Edit",
    delete: "Delete",
    ban: "Ban",
    unban: "Unban",
    createUser: "Create user",
    editUser: "Edit user",
    create: "Create",
    save: "Save",
    cancel: "Cancel",
    creating: "Creating…",
    saving: "Saving…",
    deleteTitle: "Delete user",
    deleteDesc:
      "Permanently remove {{email}}? All companions, conversations and memories will also be deleted. This cannot be undone.",
    deleteConfirm: "Delete",
    banTitle: "Ban user",
    banDesc: "Ban {{email}}? They will no longer be able to sign in.",
    unbanTitle: "Unban user",
    unbanDesc: "Unban {{email}}? They will be able to sign in again.",
    confirmBan: "Ban",
    confirmUnban: "Unban",
    empty: "No users found.",
    noResults: "No users match your filters.",
    total: "{{total}} users",
    pageInfo: "Page {{page}} of {{pages}}",
    previous: "Previous",
    next: "Next",
    created: "User created",
    updated: "User updated",
    deleted: "User deleted",
    banned: "User banned",
    unbanned: "User unbanned",
    notFound: "User not found.",
    back: "Back to users",
    detailTitle: "User details",
    detailDesc: "Account information and usage summary.",
    overview: "Account",
    usage: "Usage",
    companions: "Companions",
    conversations: "Conversations",
    messages: "Messages",
    memories: "Memories",
    grantCoins: "Grant coins", coinAmount: "Coin amount", grantNote: "Note", confirmGrant: "Confirm grant",
    grantSubscription: "Grant subscription", subscriptionPlan: "Subscription plan", selectPlan: "Select a plan",
    subscriptionEndsAt: "End date", coins: "coins", grantOperations: "Grant operations",
    noGrantOperations: "No grant operations yet.", operationType: "Operation", grantDetails: "Details",
    operator: "Operator", operationTime: "Operation time", grantFailed: "Grant failed",
    grantCoinsSuccess: "Granted {{coins}} coins", grantSubscriptionSuccess: "Granted {{plan}} and {{coins}} coins",
    behaviorTimeline: "Behavior timeline", behaviorTimelineDesc: "All recorded App actions and business events, newest first · {{total}} total", behaviorTime: "Time", behaviorEvent: "Event", behaviorDetails: "Details", behaviorContext: "Client", behaviorEmpty: "No behavior has been recorded yet.", behaviorRange: "{{from}}–{{to}} of {{total}}",
    behaviorCategories: { all: "All", navigation: "Navigation", auth: "Account", onboarding: "Onboarding", chat: "Chat", companion: "Companion", billing: "Billing", life: "Life & memories", profile: "Profile", updates: "What's new", system: "System records", general: "Other" },
  },

  invitation: { title: "Invitation settings", desc: "Configure inviter rewards for paid subscription and coin purchases.", invitedUsers: "Invited users", rewardedCoins: "Rewarded coins", rate: "Reward ratio", rateDesc: "The inviter receives this percentage of every paid coin increase. Admin grants are excluded.", percent: "Reward percentage (%)", example: "For 1,000 purchased coins, the inviter receives {{coins}} coins.", save: "Save settings", saved: "Invitation settings saved" },

  agent: {
    title: "AI companion agent",
    desc: "Configure agent routing, life generation and individual companions. Manage model services under Model services.",
    providers: "Providers",
    providersDesc: "OpenAI-compatible and Anthropic credentials. API keys are write-only and encrypted.",
    providerName: "Name",
    providerType: "Type",
    baseUrl: "Base URL",
    apiKey: "API key",
    keepSecret: "Leave blank to keep the current key",
    defaultEndpoint: "Default endpoint",
    keyConfigured: "Key configured",
    keyMissing: "Key missing",
    noProviders: "No providers configured.",
    provider: "Provider",
    models: "Models",
    modelsDesc: "Register the remote model identifiers available to companion agents.",
    modelId: "Remote model ID",
    displayName: "Display name",
    noModels: "No models configured.",
	"capability.text": "Text", "capability.image": "Image", "capability.audio": "Audio", "capability.video": "Video",
    modelScenarios: "Model scenarios", modelScenariosDesc: "Select the scenarios this model is intended to support. Availability testing is optional.", verifiedScenariosLocked: "Provider, model ID, and capabilities are locked. Add a new model to change them.", transcriptionTestFile: "Test audio", videoTestUnavailable: "Video has no test adapter yet and will be reported as unavailable.", testingModel: "Testing…", testAvailability: "Test availability", optionalTestDesc: "Testing is separate from adding. It calls the real provider and may incur charges; its result never enables or blocks Add.", modelTestsPassed: "All selected scenario tests passed", modelTestsHaveFailures: "Some scenario tests failed; review the results below", testPassed: "Passed", testFailed: "Failed", subscriptionAccess: "Subscription access", subscriptionAccessDesc: "Select one or more existing plans to make this model plan-only. Leave all unchecked for standard users.", subscriptionOnly: "Plan only", standardUsers: "Standard users", noSubscriptionPlans: "No subscription plans are configured.",
    routing: "Life Engine settings",
    routingDesc: "Configure limits for life events, proactive messages, photos, and quiet hours.",
    chatModel: "Chat model",
    lifeModel: "Life planning model",
    proactiveModel: "Proactive message model",
	imageModel: "Life photo model",
	transcriptionModel: "Speech-to-text model",
	speechModel: "Text-to-speech model",
    selectModel: "Select…",
    eventMin: "Events minimum",
    eventMax: "Events maximum",
    proactiveLimit: "Daily proactive limit",
	photoLimit: "Daily photo limit",
    quietStart: "Quiet starts",
    quietEnd: "Quiet ends",
    freeDefaultChatHours: "Free default chat (hours)",
    saveSettings: "Save routing",
    portraits: "System portraits",
    portraitsDesc: "Portraits users can choose during version-one creation.",
    portraitName: "Portrait name",
    imageUrl: "Image URL",
    gender: "Archetype",
    tags: "Personality tags",
    addPortrait: "Add portrait",
    defaultCompanion: "Use as a default free-chat companion",
    defaultCompanionBadge: "Default",
    makeDefaultCompanion: "Make default",
    removeDefaultCompanion: "Remove default",
    companions: "Companions",
    companionsDesc: "Manually configure identity, model and independent-life behavior.",
    noCompanions: "No companions found.",
    companion: "Companion",
    name: "Name",
    city: "City",
    occupation: "Occupation",
    model: "Assigned model",
    portrait: "Portrait",
    noPortrait: "No portrait",
    useDefault: "Use default chat model",
    persona: "Persona and boundaries",
    backstory: "Backstory",
    speakingStyle: "Speaking style",
    habitsGoal: "Habits (first line) and life goal",
    proactiveEnabled: "Allow bounded proactive contact",
    saveCompanion: "Save companion",
    add: "Add",
    update: "Update",
    saved: "Agent configuration saved",
  },

  companions: {
    title: "Character management", desc: "Review and manage user-created characters and their conversations.", searchPlaceholder: "Search character or user email…", allStatus: "All statuses", active: "Active", inactive: "Inactive", proactive: "Proactive", empty: "No user-created characters found.", companion: "Character", owner: "Owner", status: "Status", chats: "Chats / messages", total: "{{total}} characters", back: "Back to characters", notFound: "Character not found.", profile: "Character profile", profileDesc: "Identity, model, portrait, personality and behavior settings.", relationshipStage: "Relationship stage", interests: "Interests", lifeHabitsGoal: "Life habits (first line) and life goal", likes: "Likes", dislikes: "Dislikes", roleEnabled: "Character enabled", runtime: "Runtime state", conversations: "Conversations", messages: "Messages", memories: "Memories", lifeEvents: "Life events", intimacy: "Intimacy", enthusiasm: "Enthusiasm", mood: "Mood", energy: "Energy", createdUpdated: "Created {{created}} · Updated {{updated}}", voice: "Voice module", voiceDesc: "Reserved character-level voice settings for a later voice-message release.", voiceEnabled: "Voice capability enabled", voiceConfig: "Voice configuration (JSON)", voiceNotice: "Reserved only: the current app remains text-only and will not send voice until the delivery pipeline is released.", voiceJSONInvalid: "Voice configuration must be valid JSON.", saved: "Character saved", deleted: "Character deleted", deleteTitle: "Delete character", deleteDesc: "Permanently delete {{name}} and all conversations, messages, memories and life events? This cannot be undone.", deleteConfirm: "Delete permanently", chatHistory: "Chat history", chatHistoryDesc: "Read-only view of conversations between the user and this character.", noConversations: "No conversations yet.", conversation: "Conversation", noMessages: "No messages yet.", userAndRole: "User and {{name}}", user: "User", proactiveMessage: "Proactive outreach", replyToUser: "Reply to user", media: "Open media", older: "Older", newer: "Newer", viewUserCompanions: "View this user's characters",
  },

  billing: {
    plansTitle: "Subscription plans", plansDesc: "Configure subscription products separately by platform and environment.",
    packsTitle: "Coin packs", packsDesc: "Configure one-time coin products separately by platform and environment.",
    activityTitle: "Purchases & ledger", activityDesc: "Review purchases and coin movements within the selected platform and environment.",
    addPlan: "Add plan", addPack: "Add coin pack", purchases: "Purchases", ledger: "Coin ledger",
    environment: "Environment", allPlatforms: "All platforms", platformLabel: "Platform",
    env: { dev: "dev (development)", beta: "beta (pre-release)", prod: "prod (production)" },
    platform: { ios: "iOS", android: "Android", web: "Web", system: "System" },
    name: "Name", key: "Key", coins: "Coins", price: "Price (USD)", period: "Period", productId: "Store product ID",
    periods: { week: "Weekly", month: "Monthly", year: "Yearly" },
    status: "Status", actions: "Actions", enabled: "Enabled", disabled: "Disabled", popular: "Popular",
    edit: "Edit", delete: "Delete", save: "Save", cancel: "Cancel", sortOrder: "Sort order",
    user: "User", kind: "Kind", amount: "Paid amount", time: "Time", change: "Change", balance: "Balance", description: "Description",
    kinds: { subscription: "Subscription", coin_pack: "Coin pack" },
    empty: "No records match these filters.", formDesc: "The environment and platform identify an independent store product configuration.",
    saved: "Billing product saved", deleted: "Billing product deleted", deleteConfirm: "Delete {{name}}?",
  },

  mediaModels: { title: "Model routing", description: "Use type tabs to choose a default and ordered fallback models for each character behavior.", save: "Save routing", saved: "Model routing saved", enabled: "Behavior enabled", defaultModel: "Default model", fallbackModels: "Fallback models", addFallback: "Add fallback", noFallback: "No fallback model. Failures will stop here.", notSelected: "Not selected", noCompatibleModels: "Create and enable a compatible model on the AI companion page first.", "type.text": "Text", "type.text.desc": "Models for character creation, chat, life planning, and proactive messages.", "route.text_chat": "Character chat", "route.text_chat.desc": "Generates direct replies; a character-specific text model takes priority.", "route.text_life_plan": "Life planning", "route.text_life_plan.desc": "Generates daily Life timelines and social events between characters.", "route.text_proactive": "Proactive messages", "route.text_proactive.desc": "Generates Life event shares and memory follow-ups.", "route.text_character_profile": "Character creation", "route.text_character_profile.desc": "Generates editable character profiles from a description or uploaded document.", "route.text_story_chapter": "Story chapters", "route.text_story_chapter.desc": "Continues a branching story and creates three choices.", "route.text_storyboard": "Storyboard script", "route.text_storyboard.desc": "Distills story history into a summary and visual panels.", "type.image": "Image", "type.image.desc": "Models that create visual moments from the character's established life.", "type.audio": "Audio", "type.audio.desc": "Models that understand user audio or give the character a voice.", "type.video": "Video", "type.video.desc": "Optional models for asynchronous life clips and real-time avatar sessions.", "route.image_life_photo": "Automatic life photos", "route.image_life_photo.desc": "Creates selected photos from due Life events.", "route.image_requested_photo": "User-requested photos", "route.image_requested_photo.desc": "Creates paid photos anchored to the current Life event.", "route.image_storyboard_sheet": "Storyboard sheet", "route.image_storyboard_sheet.desc": "Renders all selected panels together in one shareable storyboard sheet.", "route.audio_transcription": "User voice transcription", "route.audio_transcription.desc": "Turns incoming voice messages into text for the character.", "route.audio_speech": "Character speech synthesis", "route.audio_speech.desc": "Turns character replies into playable voice messages.", "route.video_life_clip": "Life short videos", "route.video_life_clip.desc": "Model route reserved for asynchronous videos based on Life events.", "route.video_realtime_avatar": "Real-time video avatar", "route.video_realtime_avatar.desc": "Model route reserved for real-time avatar and video call sessions." },
  modelServices: { title: "Model services", desc: "Register the model services available to AI companions: connection, scenarios and subscription access.", create: "New model service", edit: "Edit model service", editorDesc: "Name, type, base URL and key authenticate the provider; the remote ID is the model's identifier on the provider.", empty: "No model services configured.", saved: "Model service saved", deleted: "Model service deleted", deleteConfirm: "Delete model service {{name}}?", name: "Name", kind: "Type", baseUrl: "Base URL", apiKey: "API key", keepSecret: "Leave blank to keep the current key", defaultEndpoint: "Default endpoint", keyConfigured: "Configured", keyMissing: "Missing", remoteId: "Remote ID", displayName: "Display name", enabled: "Enabled", scenarios: "System scenarios", scenariosDesc: "Select the scenarios this model supports; adjust later from “Configure scenarios”.", configureScenarios: "Configure scenarios", enableToConfigure: "Only an enabled model can be configured", testConnection: "Test connection", testing: "Testing…", connectionOk: "Connected", connectionFailed: "Connection failed", subscriptionPlans: "Subscription plans", subscriptionPlansDesc: "Select plans to make this model plan-only. Leave all unchecked for standard users.", noPlans: "No subscription plans configured.", testResults: "Test results", ok: "OK", failed: "Failed", testEndpoint: "Endpoint", availableModels: "Models available on this key", rawResponse: "Raw upstream response",  testRequest: "Request", testResponse: "Response", noScenariosToTest: "Select at least one scenario to test.", remoteIdRequired: "Enter the remote model ID first." },

  creditProducts: { title: "Credit experiences", description: "Configure the server-side price and availability of companion experiences.", product: "Product", category: "Category", coins: "Coins", enabled: "Enabled", saved: "Credit product saved" },
  aiPets: { title: "AI Pets", desc: "Configure adoptable AI pet breeds and subscription access.", create: "New breed", edit: "Edit breed", formDesc: "Disabled breeds remain available to existing owners but disappear from the adoption catalog.", name: "Pet name", species: "Breed / species", personality: "Personality", personalityHint: "gentle, curious, playful", avatarUrl: "Avatar URL", description: "Introduction", sortOrder: "Display order", subscriptionAccess: "Subscription access", subscriptionAccessDesc: "No selection allows any active subscription. Selecting plans restricts adoption to those exact store products.", add: "Add breed", update: "Save changes", saved: "AI pet breed saved", allPlans: "All active plans", selectedPlans: "{{count}} selected plans" },
  storyHub: { title: "Story Hub", desc: "Configure default story worlds, quotas, unlock rules, and coin costs.", rules: "Rules and pricing", freeLimit: "Free-user chapter limit", customLimit: "Subscriber lifetime custom-background quota", customLimitHint: "Deleting a custom background never returns quota. Users choose 4, 6, 8, or 9 panels at the same storyboard price.", unlock: "Storyboard unlock chapters", chapterCoins: "Coins per chapter", storyboardCoins: "Coins per storyboard", saveRules: "Save rules", saved: "Story settings saved", backgrounds: "Default backgrounds", create: "New background", edit: "Edit background", name: "Title", cover: "Cover URL", synopsis: "Synopsis", world: "World setting", opening: "Opening", genre: "Genre", constraints: "Character constraints", goal: "Story goal", sort: "Display order", enabled: "Enabled", saveBackground: "Save background", delete: "Delete", deleted: "Background deleted", deleteConfirm: "Delete this default background? Existing stories keep their snapshot." },

  time: {
    justNow: "just now",
    minutesAgo: "{{n}}m ago",
    hoursAgo: "{{n}}h ago",
    daysAgo: "{{n}}d ago",
  },
  content: { enabled: "Enabled", disabled: "Disabled", save: "Save", saved: "Saved", cancel: "Cancel", deleted: "Deleted", name: "Name", status: "Status", schedule: "Schedule", operator: "Operator", actions: "Actions", identifier: "Identifier", icon: "Icon", imageUrl: "Image URL", title: "Title", body: "Description", startsAt: "Starts at", endsAt: "Ends at" },
  onboarding: { title: "Onboarding", desc: "Configure the first-run introduction by environment and platform.", settings: "Delivery settings", settingsDesc: "Revision is recorded for QA and content coordination; each install completes onboarding only once.", revision: "Revision", page: "Page {{n}}", addPage: "Add page" },
  social: { title: "Social media links", desc: "Configure the official accounts shown in the Vita app.", cardTitle: "Follow Vita", cardDesc: "Only configured links appear in the Me tab for the selected environment.", hint: "Use a complete http or https URL. Leave a field empty to hide that platform.", save: "Save links", saved: "Social media links saved" },

  storage: { title: "Media storage", desc: "Store new media in Cloudflare R2 or Tencent Cloud COS for each environment.", activeProvider: "Active provider", postgresFallback: "PostgreSQL fallback", fallbackHint: "PostgreSQL is selected automatically only when both external providers are disabled.", enabled: "Enabled", endpoint: "Endpoint", bucket: "Bucket", region: "Region", accessKey: "Access key ID", secretKey: "Secret access key", keepSecret: "Leave blank to keep the saved credential.", configured: "Configured", notConfigured: "Not configured", save: "Save configuration", saved: "Storage configuration saved", test: "Test connection", tested: "Connection test succeeded", saveBeforeTest: "Save credentials before testing.", r2Desc: "Use the S3 API endpoint from your Cloudflare R2 bucket.", cosDesc: "Use the regional COS S3-compatible endpoint, for example https://cos.ap-guangzhou.myqcloud.com.", externalFailure: "When an external provider is active, upload failures are returned as errors and are not silently written to PostgreSQL." },
  legal: { title: "Legal documents", desc: "Manage the English Privacy Policy and Terms of Service by environment.", privacy: "Privacy Policy", terms: "Terms of Service", create: "New version", edit: "Edit draft", delete: "Delete", activate: "Set effective", version: "Version", summary: "Summary", content: "English content", updatedAt: "Updated", effective: "Effective", historical: "Historical", draft: "Draft", empty: "No versions in this scope.", immutableHint: "Published versions are permanent and read-only. Create a new version for every change.", englishOnly: "Only the latest effective English version is delivered to the app.", activated: "Effective version updated", deleted: "Draft deleted", saved: "Draft saved", deleteConfirm: "Delete this draft?" },
  whatsNew: { title: "What's new", desc: "Create scheduled, version-targeted update campaigns for each environment and platform.", create: "New campaign", edit: "Edit campaign", version: "Minimum app version", action: "CTA action", actionValue: "Route or URL", actionLabel: "CTA label", deleteConfirm: "Delete this campaign?", empty: "No campaigns in this scope." },
  lifeEngine: {
    title: "Life Engine",
    desc: "Observe the background worker and take over a companion manually: daily plans, proactive dispatch, push outbox, and agent runs.",
    runPlan: "Run plan pass",
    runProactive: "Run proactive dispatch",
    runningPlan: "Generating daily plans…",
    runningProactive: "Dispatching due proactive messages…",
    planDone: "Plan pass completed",
    planDoneStats: "Scanned {{scanned}} companions — newly completed {{done}}, failed {{failed}}",
    runPlanForce: "Force regenerate",
    runPlanForceHint: "Reset today's plans and rebuild all events (overwrites today's life_events).",
    proactiveDoneStats: "Due events {{due}} — dispatched {{sent}}",
    blockedInactive: "companion inactive",
    blockedLifeOff: "life engine off",
    blockedTakeover: "admin takeover",
    blockedNoSubscription: "no active subscription",
    proactiveDone: "Proactive dispatch completed",
    runFailed: "Run failed",
    activeCompanions: "Active companions",
    takeoverSub: "{{n}} under admin takeover",
    eventsToday: "Events today (shared / total)",
    eventsSharedHint: "Shared events become companion messages",
    proactiveToday: "Proactive messages today",
    outboxHint: "{{ready}} ready · {{retrying}} retrying · {{failed}} failed",
    runsFailedToday: "Failed runs (24h)",
    lastRun: "Last run {{at}}",
    neverRan: "No runs recorded yet",
    empty: "No companions in the Life Engine yet.",
    companion: "Companion",
    state: "State",
    todayEvents: "Today's events",
    dueUnshared: "Due unshared",
    mood: "Mood",
    lastRunCol: "Last run",
    automatic: "Automatic",
    inTakeover: "Admin takeover",
    lifeOff: "Life off",
    proactiveOff: "Proactive off",
    events: "Events",
    broadcast: "Message",
    resend: "Resend",
    takeover: "Take over",
    resume: "Resume",
    takeoverStarted: "Takeover started — automatic dispatch paused",
    takeoverEnded: "Takeover ended — automatic dispatch resumed",
    outboxReset: "{{n}} push row(s) queued for retry",
    eventsTitle: "Today — {{name}}",
    eventsDesc: "Events the Life Engine generated today. Manually added events use source = admin.",
    shared: "Shared",
    shareable: "Shareable",
    importance: "importance {{n}}",
    moodValue: "mood {{mood}} · energy {{energy}}",
    intimacyValue: "intimacy {{n}}",
    addEventTitle: "Add an event",
    titlePlaceholder: "e.g. Morning coffee",
    descriptionPlaceholder: "What happened (optional)",
    locationPlaceholder: "Location (optional)",
    shareableHint: "Eligible for proactive sharing",
    addEvent: "Add event",
    eventAdded: "Event added",
    eventDeleted: "Event removed",
    broadcastTitle: "Message as {{name}}",
    broadcastDesc: "Posts a message on behalf of this companion to {{email}}. Marked admin_impersonation for audit.",
    message: "Message",
    messagePlaceholder: "Write the message the companion should send…",
    conversationIdOptional: "Conversation ID (optional)",
    conversationIdPlaceholder: "Leave blank to use the latest conversation",
    cancel: "Cancel",
    send: "Send",
    broadcastSent: "Message sent",
  },
};

export default en;
