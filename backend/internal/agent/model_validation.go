...
// It records the HTTP method and URL actually used so the admin console can
// see the full request, not just the prompt.
...
func endpointFor(model Model, path string) string {
	return providerEndpoint(defaultBaseURL(model.Kind, model.BaseURL), path)
}

func chatEndpoint(model Model) string {
	if model.Kind == "anthropic" {
		return endpointFor(model, "/messages")
	}
	return endpointFor(model, "/chat/completions")
}

// TestModelScenario runs one real scenario against the provider and records
// the request and response payloads (including the HTTP target) so the admin
// console can inspect them.
func TestModelScenario(ctx context.Context, client *Client, model Model, scenario string, audio *ModelTestAudio) (ModelScenarioTest, error) {
	...
	case "text_chat", "text_story_chapter", "text_storyboard":
		...
		return textTranscript(model, req, out), err
	case "text_character_profile":
		...
		return textTranscript(model, req, out), nil
	case "text_life_plan":
		req := GenerateRequest{
		...
		if len(events) != 1 {
			return textTranscript(model, req, out), fmt.Errorf("provider returned an unexpected number of life plan events")
		}
		...
		if events[0].Title == "" || events[0].Description == "" || !startOK || !endOK {
			return textTranscript(model, req, out), fmt.Errorf("provider returned an incomplete life plan event")
		}
		return textTranscript(model, req, out), nil
	case "text_proactive":
		req := GenerateRequest{
	...
	case "image_life_photo":
		...
		return imageTranscript(model, req, out), err
	...
	case "audio_speech":
		...
		return ModelScenarioTest{
			Method:   "POST",
			URL:      endpointFor(model, "/audio/speech"),
	...
	case "audio_transcription":
		...
		return ModelScenarioTest{
			Method:   "POST",
			URL:      endpointFor(model, "/audio/transcriptions"),
...
func textTranscript(model Model, req GenerateRequest, response string) ModelScenarioTest {
	return ModelScenarioTest{
		Method:   "POST",
		URL:      chatEndpoint(model),
		Request:  fmt.Sprintf("model=%q system=%q user=%q temperature=%.2f max_tokens=%d", model.ModelName, req.System, req.Messages[0].Content, req.Temperature, req.MaxTokens),
		Response: response,
	}
}

func imageTranscript(model Model, req GenerateImageRequest, response string) ModelScenarioTest {
	return ModelScenarioTest{
		Method:   "POST",
		URL:      endpointFor(model, "/images/generations"),
		Request:  fmt.Sprintf("model=%q prompt=%q size=%s n=1", model.ModelName, req.Prompt, req.Size),
		Response: response,
	}
}
